// Block sınıfı - Blockchain'deki her bir blok (Crypto-JS versiyonu)
// Crypto-JS CDN'den yüklenerek kullanılacak
...
export class Block {
    ...
    // SHA-256 hash fonksiyonu (Crypto-JS kullanarak)
    calculateHash() {
        const str = this.previousHash + this.timestamp + JSON.stringify(this.data) + this.nonce;
        
        // Crypto-JS ile SHA-256 hash hesaplama
        if (typeof CryptoJS !== 'undefined') {
            return CryptoJS.SHA256(str).toString();
        } else {
            // Fallback: Basit hash (CryptoJS yüklenmemişse)
            console.warn('⚠️ CryptoJS bulunamadı, basit hash kullanılıyor');
