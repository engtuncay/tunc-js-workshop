...
// Blockchain sınıfı - Tüm blokları yöneten ana sınıf (Local Crypto-JS versiyonu)
export class Blockchain {
    ...
    constructor() {
        ...
        this.difficulty = 2; // Mining zorluğu (local crypto-js ile biraz artırıldı)
    ...
    createGenesisBlock() {
        ...
        const genesisBlock = new Block(Date.now(), "Genesis Block - Local Crypto-JS Edition", "0");
        console.log("✨ Genesis Block oluşturuldu - UTXO sistemi başlatıldı (Local Crypto-JS)");
        return genesisBlock;
    }
    ...
    // İşlem hash'i oluştur (Local Crypto-JS kullanarak)
    createTransactionHash(transaction) {
        ...
        if (typeof CryptoJS !== 'undefined') {
            ...
            return window.CryptoJS.SHA256(transactionString).toString().substring(0, 16);
        } else {
            // Fallback hash
            let hash = 0;
    ...
    // Başlangıç bakiyelerini oluştur (Browser için yardımcı fonksiyon)
    initializeBalances() {
        ...
        console.log('🪙 Başlangıç bakiyeleri oluşturuluyor...');
        ...
        console.log('✅ Başlangıç bakiyeleri oluşturuldu! (Local Crypto)');
    ...
    isChainValid() {
        ...
        console.log('✅ Blockchain ve UTXO set geçerli! (Local Crypto-JS Edition)');
        return true;
    }
    ...
    displayBlockchain() {
        console.log('\n=== LOCAL CRYPTO-JS BLOCKCHAIN BİLGİLERİ ===');
    ...
    displayAllBalances() {
        console.log('\n=== TÜM ADRESLERİN BAKİYELERİ (Local Crypto) ===');
